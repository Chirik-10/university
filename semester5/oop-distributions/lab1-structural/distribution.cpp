...
void mixture_characteristics(mixture_distribution m, double* M, double* D, double* gamma1, double* gamma2) {
    ...
    // Математическое ожидание смеси (всегда существует)
    *M = m.p * M1 + (1.0 - m.p) * M2;
    ...
    *D = m.p * (D1 + M1 * M1) + (1.0 - m.p) * (D2 + M2 * M2) - (*M) * (*M);
    ...
    // Проверяем, что дисперсия положительна
    if (*D <= 0) {
        *gamma1 = INFINITY;
        *gamma2 = INFINITY;
        return;
...
double empirical_density(double x, empirical_distribution emp) {
    ...
    // Ширина интервала
    double bin_width = (max_val - min_val) / emp.bin_count;

    // Определение номера интервала
    int bin_index = (int)((x - min_val) / bin_width);
...
double empirical_random(empirical_distribution emp) {
    // Нормировка плотностей для получения вероятностей
    ...
    // Генерация равномерно распределенной величины в выбранном интервале
    double bin_width = (emp.bins[1] - emp.bins[0]); // предполагаемая ширина интервала
