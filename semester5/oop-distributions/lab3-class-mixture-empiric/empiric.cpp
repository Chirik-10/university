...
Empiric::Empiric(const double* data_arr, int n0, int k0)
    : n(0), k(0), data(nullptr), bins(nullptr), densities(nullptr), min_val(0.0), max_val(0.0)
{
...
// Конструктор из основного распределения
...
// Конструктор из смеси распределений
...
// Конструктор из эмпирического распределения
...
double Empiric::density(double x) const {
    double min_val = bins[0] - (bins[1] - bins[0]) / 2;
    double max_val = bins[k - 1] + (bins[k - 1] - bins[k - 2]) / 2;

    if (x < min_val || x > max_val) {
        return 0.0;
    }

    double bin_width = (max_val - min_val) / k; // ширина интервала
    int bin_index = (int)((x - min_val) / bin_width); // номер интервала с точкой
