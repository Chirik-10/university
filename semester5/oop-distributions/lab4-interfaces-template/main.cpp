...
void demo_mixture_class() {
    ...
    try {
        ...
        Mixture<Primary, Primary> mix(prim1, prim2, p);
...
// Функция для тестирования позднего связывания
void test_late_binding() {
    ...
    Empiric emp(100, prim, 20);

    // 2. Массив указателей на интерфейс
    IDistribution* distributions[3];
    distributions[0] = &prim;
    distributions[1] = &mix;
    distributions[2] = &emp;

    printf("1. Вызов виртуальных функций через IDistribution*:\n");
    for (int i = 0; i < 3; i++) {
        printf("   Объект %d:\n", i + 1);
        printf("   - density(1.0) = %f\n", distributions[i]->density(1.0));
        printf("   - random() = %f\n", distributions[i]->random());
    }

    // 3. Глубокая смесь
    printf("\n2. Демонстрация шаблонности Mixture (глубокая смесь):\n");
    Mixture<Mixture<Primary, Primary>, Primary> deepMix(
        Mixture<Primary, Primary>(Primary(0, 1, 3), Primary(2, 1, 4), 0.5),
        Primary(1, 2, 5),
        0.3
    );
    printf("   Создана глубокая смесь (смесь смесей)\n");
    printf("   Параметр смеси: p = %f\n", deepMix.getP());

    // 4. Empiric из глубокой смеси
    Empiric empFromDeep(200, deepMix, 15);
    printf("   Создан Empiric из глубокой смеси\n");
    printf("   Размер выборки: %d\n", empFromDeep.getSize());

    // 5. Сохранение через интерфейс
    printf("\n3. Демонстрация персистентности через IPersistent*:\n");
    IPersistent* persistents[3];
    persistents[0] = &prim;
    persistents[1] = &mix;
    persistents[2] = &emp;
    ...
    for (int i = 0; i < 3; i++) {
        char filename[50];
        sprintf_s(filename, "test_persist_%d.txt", i);
        FILE* file;
        fopen_s(&file, filename, "w");
        if (file) {
            persistents[i]->save(file);
            fclose(file);
            printf("   Объект %d сохранен в %s\n", i + 1, filename);
        }
    }
